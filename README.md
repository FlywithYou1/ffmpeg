...
- **软件编解码器**: libmp3lame (MP3), libfdk-aac (AAC), libx264 (H.264), libx265 (HEVC), libvpx (VP8/VP9), libopus, libvorbis, libtheora, libaom (AV1), libsvtav1 (AV1), libdav1d (AV1 解码), libkvazaar (HEVC), libopenh264, libxvid, libtwolame, libspeex, libcodec2, libjxl (JPEG XL), libopenmpt, libwavpack, libbs2b, libaribb24, libplacebo, librubberband, libvidstab, libwebp, libass (字幕), libfreetype, fontconfig, libzimg, libsoxr, libopenjpeg, libsnappy, SDL2 (ffplay)
...
sudo apt-get install -y build-essential yasm nasm pkg-config meson ninja-build \
  ...
  libsvtav1-dev libdav1d-dev libkvazaar-dev libopenh264-dev libxvidcore-dev \

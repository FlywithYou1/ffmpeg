#!/usr/bin/env bash
# ============================================================
# FFmpeg + Intel QSV (oneVPL) + VAAPI + VMAF (Ubuntu)
# ============================================================
set -Eeuo pipefail
...
./configure --prefix="$P" --pkg-config-flags="--static" \
  ...
  --enable-libvmaf --enable-libvpl --enable-vaapi \
  ...
  --enable-libsvtav1 --enable-libdav1d --enable-libkvazaar --enable-libopenh264 --enable-libxvid --enable-libtwolame --enable-libspeex --enable-libcodec2 --enable-libjxl --enable-libopenmpt --enable-libbs2b --enable-libaribb24 --enable-librubberband --enable-libvidstab \
...
echo "--- QSV ---"
"$P/bin/ffmpeg" -hide_banner -encoders 2>&1 | grep '_qsv' | head -10 || echo "(none)"
echo "--- VAAPI ---"

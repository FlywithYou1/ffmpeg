...
if [ -n "${CLANG_BIN:-}" ] && [ -f "${CLANG_BIN}/clang.exe" ]; then
  ...
  VMAF_C_ARGS=$(__meson_array "-MT" "-I${P_MIXED}/include" "-I${CUDA_HOME_MIXED}/include" "-D_USE_MATH_DEFINES" ${PTHREAD_CFLAGS:+"$PTHREAD_CFLAGS"})
  VMAF_LINK_ARGS=$(__meson_array ${PTHREAD_LDFLAGS:+"$PTHREAD_LDFLAGS"})
  PKG_CONFIG_PATH="$P_MIXED/lib/pkgconfig;${PKG_CONFIG_PATH:-}" \
...
./configure --toolchain=msvc --prefix="$P" \
  --extra-cflags="-MT -I${P_MIXED}/include -I${CUDA_HOME_MIXED}/include ${VCPKG_CFLAGS}" \
